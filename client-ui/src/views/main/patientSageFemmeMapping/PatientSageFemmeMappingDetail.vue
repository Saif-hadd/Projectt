  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="patientsagefemmemapping">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(patientsagefemmemapping)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="patientsagefemmemappingDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { patientsagefemmemapping } = storeToRefs(
  usePatientSageFemmeMappingStore()
);
const { getPatientSageFemmeMappingById } = usePatientSageFemmeMappingStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getPatientSageFemmeMappingById(id);
  setCurrentPageBreadcrumbs("patientsagefemmemappings", ["detail"]);
});
</script>
<style>
.bottom {
