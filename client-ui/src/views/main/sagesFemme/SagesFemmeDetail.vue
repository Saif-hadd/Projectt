  ...
  <el-row>
    <el-col :xs="24" :md="12" class="mx-auto mb-4">
      <el-card :body-style="{ padding: '0px' }">
        ...
        <div style="padding: 14px" v-if="sagesfemme">
          <el-row
            :gutter="10"
            v-for="(item, index) in Object.entries(sagesfemme)"
            :key="index"
          >
          ...
          <div class="bottom">
            <el-button
              :data-test="sagesfemmeDetailBackBtn"
              type="primary"
              @click="() => $router.go(-1)"
              icon="ArrowLeft"
...
const { sagesfemme } = storeToRefs(useSagesFemmeStore());
const { getSagesFemmeById } = useSagesFemmeStore();
onMounted(async () => {
  const id = route?.params?.id as string;
  await getSagesFemmeById(id);
  setCurrentPageBreadcrumbs("sagesfemmes", ["detail"]);
});
</script>
<style>
.bottom {
